...
#     # input_dir = Path("/app/input")  # Changed to match Docker volume
#     # output_dir = Path("/app/output")  # Changed to match Docker volume
...
def process_pdfs():
    input_dir = Path("/app/input")  # Changed to match required Docker format
    output_dir = Path("/app/output")  # Changed to match required Docker format
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    for pdf_file in input_dir.glob("*.pdf"):
        ...
        output_file = output_dir / f"{pdf_file.stem}.json"
        with open(output_file, "w") as f:
            json.dump(PDFOutline(**structure).model_dump(), f, indent=2)  # Updated to model_dump()

if __name__ == "__main__":
    print("=== Starting PDF Processor ===")
    print(f"Current directory: {Path.cwd()}")
    print(f"Input directory exists: {Path('./input').exists()}")
    print(f"Input files: {list(Path('./input').glob('*'))}")
    process_pdfs()